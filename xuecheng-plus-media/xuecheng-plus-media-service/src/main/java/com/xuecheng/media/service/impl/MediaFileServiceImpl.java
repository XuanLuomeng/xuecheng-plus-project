...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public MediaFiles addMediaFilesToDb(Long companyId, String fileMd5, UploadFileParamsDto uploadFileParamsDto, String bucket, String objectName) {
        ...
        if (mediaFiles == null) {
            ...
            //通过mimeType判断如果是avi判断如果是avi视频写入待处理任务
            //向MediaProcess插入记录
            return mediaFiles;
        }
        return mediaFiles;
    ...
    private void addWaitingTask(MediaFiles mediaFiles) {
        ...
        String extension = filename.substring(filename.lastIndexOf("."));
        String mimeType = getMimeType(extension);
        if (mimeType.equals("video/x-msvideo")) {//如果avi视频写入待处理任务
    ...
    public RestResponse mergechunks(Long companyId, String fileMd5, int chunkTotal, UploadFileParamsDto uploadFileParamsDto) {
        ...
        try (FileInputStream fileInputStream = new FileInputStream(file)) {
            ...
            //文件大小
            uploadFileParamsDto.setFileSize(file.lastModified());
        } catch (Exception e) {
            return RestResponse.validfail(false, "文件校验失败");
        }
        //===========将文件信息入库===========
    ...
    private void clearChunkFiles(String chunkFileFolderPath, int chunkTotal) {
        Iterable<DeleteObject> objects = Stream.iterate(0, i -> ++i).map(i -> new DeleteObject(chunkFileFolderPath.concat(Integer.toString(i)))).collect(Collectors.toList());
